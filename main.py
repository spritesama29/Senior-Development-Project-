...
def add250TV(cursor: sqlite3.Cursor, tv):
    ...
    if len(check.fetchall()) == 0:
        q = "INSERT INTO table250(show_id, rank, title,full_title,year,crew,imdb_rating,rating_count) VALUES (?,?,?,?,?,?,?,?)"
        for i in range(250):
...
def add250MOV(cursor: sqlite3.Cursor, tv):
    q = "INSERT INTO table250MOV(show_id, rank, title,full_title,year,crew,imdb_rating,rating_count) VALUES (?,?,?,?,?,?,?,?)"
    for i in range(250):
        cursor.execute(q, (((tv.get("items"))[i]).get("id"), ((tv.get("items"))[i]).get("rank"),
                       ((tv.get("items"))[i]).get("title"),
                       ((tv.get("items"))[i]).get("fullTitle"), ((tv.get("items"))[i]).get("year"),
...
def addPopMOV(cursor: sqlite3.Cursor, tv):
    q = "INSERT INTO popularMOV(show_id,rank,rankUpDown,title,full_title,year,crew,imdb_rating,rating_count) VALUES (?,?,?,?,?,?,?,?,?)"
    for i in range(100):
        cursor.execute(q, (((tv.get("items"))[i]).get("id"), ((tv.get("items"))[i]).get("rank"),
                           ((tv.get("items"))[i]).get("rankUpDown"), ((tv.get("items"))[i]).get("title"),
                           ((tv.get("items"))[i]).get("fullTitle"), ((tv.get("items"))[i]).get("year"),
 ...
 #   data1 = getUserRatings(findIdbyRank("1", tv))
  #  data50 = getUserRatings(findIdbyRank("50", tv))
   # data100 = getUserRatings(findIdbyRank("100", tv))
    ...
    tv = getMostPopularMovies()
    dic = orderRankUpDownMOV(tv)
    a = getUserRatings(dic[0][0])
    b = getUserRatings(dic[97][0])
    c = getUserRatings(dic[98][0])
    d = getUserRatings(dic[99][0])
    addRatingsMOV(cursor,a)
    addRatingsMOV(cursor,b)
    addRatingsMOV(cursor,c)
    addRatingsMOV(cursor,d)
    #addPopMOV(cursor,tv)
    #add250MOV(cursor, tv)
    #tv = getMostPopularTVs()
    #addPopTV(cursor,tv)
    #add250TV(cursor, tv)
